<?php	
    ...
    $query_buscar="SELECT nombre_marca FROM marca;";
    $result=mysqli_query($enlace,$query_buscar);
    while( ( $row=mysqli_fetch_array($result,MYSQLI_ASSOC) ) && ($sw==1) ){
        if(strcasecmp($row['nombre_marca'],$marca)==0) {
            $sw=0;
        }
    }
	
    if($sw==1){
    	$query="INSERT INTO `marca` (`nombre_marca`,`descripcion_marca`) VALUES ('$marca','$desc');";
